...
6. [Visualizations & Figures Index](#5-visualizations--figures-index)
7. [Policy Implications & Conclusion](#6-policy-implications--conclusion)
...
*Source: [`analysis/results/part_a_table1_descriptives.csv`](analysis/results/part_a_table1_descriptives.csv). Note: The unmatched $\sim 25\%$ in Scopus comprises conference proceedings, book series, and trade reports that structurally lack ISSNs.*
...
In parallel, we model graduate career destinations as two symmetric, non-artifactual equations (deliberately kept separate — see Section 3.3 note on why one does not predict the other):
...
> **Specification note**: `placement_rate_pct` and `higher_studies_rate_pct` are both computed from the same `total_graduating` denominator — a graduate who enters higher studies is, near-mechanically, a graduate not counted as "placed." An earlier specification used `higher_studies_rate_pct` as a *predictor* of placement rate, which risked reporting an arithmetic artifact of the shared denominator as a causal finding. The two are now modeled as **parallel, independent outcomes** (Model A2 and Model A2b), both driven by the same explanatory variables, rather than one predicting the other.
...
*Source: [`analysis/results/part_b_table1_quartile_trends.csv`](analysis/results/part_b_table1_quartile_trends.csv).*
...
All generated high-resolution ($300\text{ dpi}$) figures are saved in [`analysis/results/figures/`](analysis/results/figures/).

### Figure 1 — PhD Scholars vs. Research Output
Scatter of $\ln(\text{PhD Scholars})$ vs. $\ln(\text{Scopus Volume})$, with tier coloring and annotations for landmark universities (*IISc, JNU, BHU, Anna Univ, Jamia Millia, Manipal, AMU*).

![Figure 1: PhD Scholars vs. Scopus Output](analysis/results/figures/fig1_phd_vs_scopus_output.png)

### Figure 2 — Faculty Ratio vs. Placement Rate
...
$\ln(\text{Volume})$ vs. `% Q1 Share`, illustrating the (positive, not negative) slope that refutes the gaming hypothesis.
